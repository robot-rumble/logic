...
class _RobotRumbleLoggingIO(io.TextIOBase):
    ...
    def write(self, s):
        if self._log:
            ...
            raise RuntimeError("unable to print; the current runner does not support logging")
        return len(s)
    def flush(self):
        pass
del io
...
def _main(state, log=None):
    ...
    if robot.__code__.co_argcount != 2:
        raise TypeError("your robot function must accept 2 values: the current state \
                         and unit details")
    output = {}
